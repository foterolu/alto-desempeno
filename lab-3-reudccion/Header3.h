...
__global__ void reduccion(int* conejos, int* suma_conejos, int* lobos, int* suma_lobos, int array_size) {
    ...
    __syncthreads();
    // Los indices a utilizar son distintos, esto permite que los threads accedan a memoria de manera coalescente
    ...
    if (tid == 0) {
        suma_conejos[blockIdx.x] = conejos_shared[0];
        suma_lobos[blockIdx.x] = lobos_shared[0];
    }
}